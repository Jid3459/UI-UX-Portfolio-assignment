...
function tokens(darkMode: boolean) {
  return {
...
function SectionRow({ marker, children, reverse }: { marker: ReactNode; children: ReactNode; reverse?: boolean }) {
  return (
...
function SectionMarker({ icon, title, index, t, accent = "#3fb950" }: { icon: ReactNode; title: string; index: number; t: Tokens; accent?: string }) {
...
function ProcessTimeline({ steps, t }: { steps: CaseStudy["process"]; t: Tokens }) {
  return (
    <div className="relative">
...
function ScreenBlock({ screen, index, darkMode, t, figma }: { screen: ScreenComparison; index: number; darkMode: boolean; t: Tokens; figma?: string }) {
  const padded = String(index).padStart(2, "0");
  return (
...
function Pill({ label, color, border }: { label: string; color: string; border: string }) {
  return (
    <span
      style={{
        color,
